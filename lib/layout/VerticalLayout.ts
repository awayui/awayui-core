...
module feathers.layout
{
	 ...
	 */
	export class VerticalLayout extends EventDispatcher implements IVariableVirtualLayout, ITrimmedVirtualLayout
	{
		/**
		 ...
		 * @private
		 */
		protected _verticalAlign:string = VerticalLayout.VERTICAL_ALIGN_TOP;
		 ...
		 * @private
		 */
		protected _horizontalAlign:string = VerticalLayout.HORIZONTAL_ALIGN_LEFT;
		 ...
		 * @private
		 */
		protected _distributeHeights:boolean = false;
		 ...
		 * @default false
		 */
		public get distributeHeights():boolean
		{
		 ...
		 * @private
		 */
		public set distributeHeights(value:boolean)
		{
		 ...
		 * @private
		 */
		protected _scrollPositionVerticalAlign:string = VerticalLayout.VERTICAL_ALIGN_MIDDLE;
		 ...
		 * @see #VERTICAL_ALIGN_BOTTOM
		 */
		public get scrollPositionVerticalAlign():string
		{
		 ...
		 * @private
		 */
		public set scrollPositionVerticalAlign(value:string)
		{
		...
		public layout(items:DisplayObject[], viewPortBounds:ViewPortBounds = null, result:LayoutBoundsResult = null):LayoutBoundsResult
		{
			...
			if(!this._useVirtualLayout || this._hasVariableItemDimensions || this._distributeHeights ||
				this._horizontalAlign != VerticalLayout.HORIZONTAL_ALIGN_JUSTIFY ||
			...
			for(var i:number = 0; i < itemCount; i++)
			{
				...
				var iNormalized:number = i + indexOffset;
			...
			if(totalHeight < availableHeight)
			{
				...
				if(this._verticalAlign == VerticalLayout.VERTICAL_ALIGN_BOTTOM)
				{
					verticalAlignOffsetY = availableHeight - totalHeight;
				}
				else if(this._verticalAlign == VerticalLayout.VERTICAL_ALIGN_MIDDLE)
				{
			...
			for(i = 0; i < discoveredItemCount; i++)
			{
				...
				if(this._horizontalAlign == VerticalLayout.HORIZONTAL_ALIGN_JUSTIFY)
				{
				...
				else
				{
					if(layoutItem)
					{
						var layoutData:VerticalLayoutData = <VerticalLayoutData>layoutItem.layoutData ;
						if(layoutData)
						{
					...
					{
						case VerticalLayout.HORIZONTAL_ALIGN_RIGHT:
						{
							item.x = item.pivotX + boundsX + availableWidth - this._paddingRight - item.width;
							break;
						}
						case VerticalLayout.HORIZONTAL_ALIGN_CENTER:
						{
			...
			result.contentX = 0;
			result.contentWidth = this._horizontalAlign == VerticalLayout.HORIZONTAL_ALIGN_JUSTIFY ? availableWidth : totalWidth;
			result.contentY = 0;
		...
		public getVisibleIndicesAtScrollPosition(scrollX:number, scrollY:number, width:number, height:number, itemCount:number, result:number[] = null):number[]
		{
			...
			var maxVisibleTypicalItemCount:number = Math.ceil(height / (calculatedTypicalItemHeight + this._gap)) + 1;
			if(!this._hasVariableItemDimensions)
			{
				...
				var minimum:number = (scrollY - this._paddingTop) / (calculatedTypicalItemHeight + this._gap);
				if(minimum < 0)
				{
					minimum = 0;
				}
		...
		public getScrollPositionForIndex(index:number, items:DisplayObject[], x:number, y:number, width:number, height:number, result:Point = null):Point
		{
			...
			if(this._scrollPositionVerticalAlign == VerticalLayout.VERTICAL_ALIGN_MIDDLE)
			{
				maxScrollY -= Math.round((height - itemHeight) / 2);
			}
			else if(this._scrollPositionVerticalAlign == VerticalLayout.VERTICAL_ALIGN_BOTTOM)
			{
		 ...
		 * @private
		 */
		protected validateItems(items:DisplayObject[], explicitWidth:number,
			minWidth:number, maxWidth:number, distributedHeight:number):void
		{
			...
			var isJustified:boolean = this._horizontalAlign == VerticalLayout.HORIZONTAL_ALIGN_JUSTIFY;
			var itemCount:number = items.length;
			for(var i:number = 0; i < itemCount; i++)
			{
				var item:DisplayObject = items[i];
		 ...
		 * @private
		 */
		protected calculateDistributedHeight(items:DisplayObject[], explicitHeight:number, minHeight:number, maxHeight:number):number
		{
			var itemCount:number = items.length;
		 ...
		 * @private
		 */
		protected applyPercentHeights(items:DisplayObject[], explicitHeight:number, minHeight:number, maxHeight:number):void
		{
			...
			do
			{
				...
				for(i = 0; i < pushIndex; i++)
				{
					...
					layoutData = this.VerticalLayoutData(layoutItem.layoutData);
