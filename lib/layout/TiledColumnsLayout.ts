...
module feathers.layout
{
	 ...
	 */
	export class TiledColumnsLayout extends EventDispatcher implements IVirtualLayout
	{
		/**
		 ...
		 * @private
		 */
		protected _verticalAlign:string = TiledColumnsLayout.VERTICAL_ALIGN_TOP;
		 ...
		 * @private
		 */
		protected _horizontalAlign:string = TiledColumnsLayout.HORIZONTAL_ALIGN_CENTER;
		 ...
		 * @private
		 */
		protected _tileVerticalAlign:string = TiledColumnsLayout.TILE_VERTICAL_ALIGN_MIDDLE;
		 ...
		 * @private
		 */
		protected _tileHorizontalAlign:string = TiledColumnsLayout.TILE_HORIZONTAL_ALIGN_CENTER;
		 ...
		 * @private
		 */
		protected _paging:string = TiledColumnsLayout.PAGING_NONE;
		...
		public layout(items:DisplayObject[], viewPortBounds:ViewPortBounds = null, result:LayoutBoundsResult = null):LayoutBoundsResult
		{
			...
			for(i = 0; i < itemCount; i++)
			{
				...
				if(itemIndex == nextPageStartIndex)
				{
					...
					if(this._paging != TiledColumnsLayout.PAGING_NONE)
					{
					...
					if(this._paging == TiledColumnsLayout.PAGING_HORIZONTAL)
					{
						positionX = startX + availableWidth * pageIndex;
					}
					else if(this._paging == TiledColumnsLayout.PAGING_VERTICAL)
					{
						positionX = startX;
				...
				if(item)
				{
					...
					{
						case TiledColumnsLayout.TILE_HORIZONTAL_ALIGN_JUSTIFY:
						{
							item.x = item.pivotX + positionX;
							item.width = tileWidth;
							break;
						}
						case TiledColumnsLayout.TILE_HORIZONTAL_ALIGN_LEFT:
						{
							item.x = item.pivotX + positionX;
							break;
						}
						case TiledColumnsLayout.TILE_HORIZONTAL_ALIGN_RIGHT:
						{
					...
					{
						case TiledColumnsLayout.TILE_VERTICAL_ALIGN_JUSTIFY:
						{
							item.y = item.pivotY + positionY;
							item.height = tileHeight;
							break;
						}
						case TiledColumnsLayout.TILE_VERTICAL_ALIGN_TOP:
						{
							item.y = item.pivotY + positionY;
							break;
						}
						case TiledColumnsLayout.TILE_VERTICAL_ALIGN_BOTTOM:
						{
			...
			if(availableWidth === availableWidth) //!isNaN
			{
				if(this._paging == TiledColumnsLayout.PAGING_VERTICAL)
				{
					totalWidth = availableWidth;
				}
				else if(this._paging == TiledColumnsLayout.PAGING_HORIZONTAL)
				{
			...
			if(availableHeight === availableHeight && //!isNaN
				this._paging == TiledColumnsLayout.PAGING_VERTICAL)
			{
		 ...
		 * @inheritDoc
		 */
		public getScrollPositionForIndex(index:number, items:DisplayObject[],
			x:number, y:number, width:number, height:number, result:Point = null):Point
		{
		...
		protected applyHorizontalAlign(items:DisplayObject[], startIndex:number, endIndex:number, totalItemWidth:number, availableWidth:number):void
		{
			...
			if(this._horizontalAlign == TiledColumnsLayout.HORIZONTAL_ALIGN_RIGHT)
			{
				horizontalAlignOffsetX = availableWidth - totalItemWidth;
			}
			else if(this._horizontalAlign != TiledColumnsLayout.HORIZONTAL_ALIGN_LEFT)
			{
		...
		protected applyVerticalAlign(items:DisplayObject[], startIndex:number, endIndex:number, totalItemHeight:number, availableHeight:number):void
		{
			...
			if(this._verticalAlign == TiledColumnsLayout.VERTICAL_ALIGN_BOTTOM)
			{
				verticalAlignOffsetY = availableHeight - totalItemHeight;
			}
			else if(this._verticalAlign == TiledColumnsLayout.VERTICAL_ALIGN_MIDDLE)
			{
		...
		protected getVisibleIndicesAtScrollPositionWithHorizontalPaging(scrollX:number, scrollY:number, width:number, height:number, itemCount:number, result:number[]):void
		{
			...
			if(totalRowWidth < width)
			{
				...
				else if(this._horizontalAlign == TiledColumnsLayout.HORIZONTAL_ALIGN_CENTER)
				{
		...
		protected getVisibleIndicesAtScrollPositionWithVerticalPaging(scrollX:number, scrollY:number, width:number, height:number, itemCount:number, result:number[]):void
		{
			...
			if(totalColumnHeight < height)
			{
				...
				else if(this._horizontalAlign == TiledColumnsLayout.VERTICAL_ALIGN_MIDDLE)
				{
					topSideOffset = bottomSideOffset = Math.round((height - this._paddingTop - this._paddingBottom - totalColumnHeight) / 2);
				}
				else if(this._horizontalAlign == TiledColumnsLayout.VERTICAL_ALIGN_TOP)
				{
					topSideOffset = 0;
			...
			else
			{
				...
				var rowIndex:number = (verticalTileCount + rowOffset) % verticalTileCount;
		...
		protected getVisibleIndicesAtScrollPositionWithoutPaging(scrollX:number, scrollY:number, width:number, height:number, itemCount:number, result:number[]):void
		{
			...
			var horizontalTileCount:number = Math.ceil((width + this._horizontalGap) / (tileWidth + this._horizontalGap)) + 1;
			var minimumItemCount:number = verticalTileCount * horizontalTileCount;
			if(minimumItemCount > itemCount)
			{
				minimumItemCount = itemCount;
			}
			var columnIndexOffset:number = 0;
			...
			var columnIndex:number = -columnIndexOffset + Math.floor((scrollX - this._paddingLeft + this._horizontalGap) / (tileWidth + this._horizontalGap));
			var minimum:number = columnIndex * verticalTileCount;
			if(minimum < 0)
			{
				minimum = 0;
			}
		 ...
		 * @private
		 */
		protected calculateScrollPositionForIndex(index:number, items:DisplayObject[], x:number, y:number,
