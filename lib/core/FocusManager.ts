...
module feathers.core
{
	...
	export class FocusManager
	{
		 ...
		 * @private
		 */
		protected static FOCUS_MANAGER_NOT_ENABLED_ERROR:string = "The specified action is not permitted when the focus manager is not enabled.";

		/**
		 * @private
		 */
		protected static FOCUS_MANAGER_ROOT_MUST_BE_ON_STAGE_ERROR:string = "A focus manager may not be added or removed for a display object that is not on stage.";

		/**
		 * @private
		 */
		protected static STAGE_TO_STACK:Dictionary = new Dictionary(true);
		 ...
		 */
		public static getFocusManagerForStage(stage:Stage):IFocusManager
		{
			var stack:IFocusManager[] = /*STAGE_TO_STACK[stage] as Vector.<IFocusManager>*/;
			if(!stack)
			{
				return null;
			}
		 ...
		 */
		public static defaultFocusManagerFactory(root:DisplayObjectContainer):IFocusManager
		{
		 ...
		 */
		public static focusManagerFactory:Function = FocusManager.defaultFocusManagerFactory;
		 ...
		 */
		public static isEnabledForStage(stage:Stage):boolean
		{
			var stack:IFocusManager[] = FocusManager.STAGE_TO_STACK[stage];
			return stack != null;
		}
		 ...
		 * @see #getFocusManagerForStage()
		 */
		public static setEnabledForStage(stage:Stage, isEnabled:boolean):void
		{
			var stack:IFocusManager[] = FocusManager.STAGE_TO_STACK[stage];
			...
			if(isEnabled)
			{
				FocusManager.STAGE_TO_STACK[stage] = new Array<IFocusManager>();
				FocusManager.pushFocusManager(stage);
			}
			else
			{
				...
				delete FocusManager.STAGE_TO_STACK[stage];
			}
		}
		...
		{
			for(var key:Object in FocusManager.STAGE_TO_STACK)
			{
		 ...
		 * @see #getFocusManagerForStage()
		 */
		public static get focus():IFocusDisplayObject
		{
			var manager:IFocusManager = FocusManager.getFocusManagerForStage(Starling.current.stage);
			if(manager)
			{
		 ...
		 * @private
		 */
		public static set focus(value:IFocusDisplayObject)
		{
			var manager:IFocusManager = FocusManager.getFocusManagerForStage(Starling.current.stage);
			if(!manager)
			{
				throw new Error(FocusManager.FOCUS_MANAGER_NOT_ENABLED_ERROR);
			}
			manager.focus = value;
		}
		 ...
		 */
		public static pushFocusManager(root:DisplayObjectContainer):IFocusManager
		{
			var stage:Stage = root.stage;
			if(!stage)
			{
				throw new ArgumentError(FocusManager.FOCUS_MANAGER_ROOT_MUST_BE_ON_STAGE_ERROR);
			}
			var stack:IFocusManager[] = /*STAGE_TO_STACK[stage] as Vector.<IFocusManager>*/;
			if(!stack)
			{
		 ...
		 * enabled.
		 */
		public static removeFocusManager(manager:IFocusManager):void
		{
			...
			var index:number = stack.indexOf(manager);
			if(index < 0)
			{
				return;
			}
