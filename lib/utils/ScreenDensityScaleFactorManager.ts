...
module feathers.utils
{
	 ...
	 */
	export class ScreenDensityScaleFactorManager
	{
		/**
		 * @private
		 */
		protected static IOS_TABLET_DENSITY_SCALE_FACTOR:number = 1.23484848484848;
		...
		constructor(starling:Starling)
		{
			...
			nativeStage.addEventListener(Event.RESIZE, this.nativeStage_resizeHandler, false, int.MAX_VALUE, true);
		}
		 ...
		 * @private
		 */
		protected _calculatedScaleFactor:number;
		...
		protected calculateScaleFactor():number
		{
			...
			{
				screenDensity *= ScreenDensityScaleFactorManager.IOS_TABLET_DENSITY_SCALE_FACTOR;
			}
			var bucket:ScreenDensityBucket = this.BUCKETS[0];
			if(screenDensity <= bucket.density)
			{
				return bucket.scale;
			}
			var previousBucket:ScreenDensityBucket = bucket;
			var bucketCount:number = this.BUCKETS.length;
			for(var i:number = 1; i < bucketCount; i++)
			{
		 ...
		 * @private
		 */
		protected updateStarlingStageDimensions():void
		{
			var nativeStage:Stage = this._starling.nativeStage;
		 ...
		 * @private
		 */
		protected nativeStage_resizeHandler(event:Event):void
		{
			this.updateStarlingStageDimensions();
		}
	}
...
{
	constructor(dpi:number, scale:number)
	{
		this.density = dpi;
		this.scale = scale;
	}
