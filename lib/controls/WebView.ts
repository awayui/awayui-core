...
module feathers.controls
{
	 ...
	 */
	export class WebView extends FeathersControl
	{
		/**
		 * @private
		 */
		 ...
		 * @private
		 */
		protected static STAGE_WEB_VIEW_NOT_SUPPORTED_ERROR:string = "Feathers WebView is only supported in Adobe AIR. It cannot be used in Adobe Flash Player.";

		/**
		 * @private
		 */
		protected static USE_NATIVE_ERROR:string = "The useNative property may only be set before the WebView component validates for the first time.";

		/**
		 * @private
		 */
		protected static DEFAULT_SIZE:number = 320;

		/**
		 * @private
		 */
		protected static STAGE_WEB_VIEW_FULLY_QUALIFIED_CLASS_NAME:string = "flash.media.StageWebView";

		/**
		 * @private
		 */
		protected static STAGE_WEB_VIEW_CLASS:Class;
		...
		{
			if(!WebView.STAGE_WEB_VIEW_CLASS)
			{
				try
				{
					WebView.STAGE_WEB_VIEW_CLASS = Class(getDefinitionByName(WebView.STAGE_WEB_VIEW_FULLY_QUALIFIED_CLASS_NAME));
				}
				catch(error:Error)
				{
					return false;
			...
			return WebView.STAGE_WEB_VIEW_CLASS.isSupported;
		}
		...
		constructor()
		{
			this.addEventListener(Event.ADDED_TO_STAGE, this.webView_addedToStageHandler);
			this.addEventListener(Event.REMOVED_FROM_STAGE, this.webView_removedFromStageHandler);
		}

		/**
		 * @private
		 */
		protected stageWebView:Object;

		/**
		 * @private
		 */
		protected _useNative:boolean = false;
		 ...
		 * @private
		 */
		public set useNative(value:boolean)
		{
			if(this.isCreated)
			{
				throw new IllegalOperationError(WebView.USE_NATIVE_ERROR);
			}
		 ...
		 */
		public loadString(text:string, mimeType:string = "text/html"):void
		{
			this.validate();
		...
		protected autoSizeIfNeeded():boolean
		{
			...
			if(needsWidth)
			{
				newWidth = WebView.DEFAULT_SIZE;
			}
			var newHeight:number = this.explicitWidth;
			if(needsHeight)
			{
				newHeight = WebView.DEFAULT_SIZE;
			}
		 ...
		 * with a custom implementation.</p>
		 */
		protected createStageWebView():void
		{
			if(WebView.isSupported)
			{
				this.stageWebView = new WebView.STAGE_WEB_VIEW_CLASS(this._useNative);
			}
			else
			{
				throw new IllegalOperationError(WebView.STAGE_WEB_VIEW_NOT_SUPPORTED_ERROR);
			}
			this.stageWebView.addEventListener(ErrorEvent.ERROR, this.stageWebView_errorHandler);
			this.stageWebView.addEventListener("locationChange", this.stageWebViewHandler);
			this.stageWebView.addEventListener(this.flash.events.Event.COMPLETE, this.stageWebViewHandler);
		}
		...
		protected refreshViewPort():void
		{
			...
			WebView.HELPER_POINT.x = WebView.HELPER_POINT.y = 0;
			this.getTransformationMatrix(this.stage, WebView.HELPER_MATRIX);
			var globalScaleX:number = matrixToScaleX(WebView.HELPER_MATRIX);
			var globalScaleY:number = matrixToScaleY(WebView.HELPER_MATRIX);
			MatrixUtil.transformCoords(WebView.HELPER_MATRIX, 0, 0, WebView.HELPER_POINT);
			...
			stageWebViewViewPort.x = Math.round(starlingViewPort.x + WebView.HELPER_POINT.x * scaleFactor);
			stageWebViewViewPort.y = Math.round(starlingViewPort.y + WebView.HELPER_POINT.y * scaleFactor);
		 ...
		 * @private
		 */
		protected webView_addedToStageHandler(event:Event):void
		{
			this.stageWebView.stage = Starling.current.nativeStage;
			this.addEventListener(Event.ENTER_FRAME, this.webView_enterFrameHandler);
		}

		/**
		 * @private
		 */
		protected webView_removedFromStageHandler(event:Event):void
		{
			if(this.stageWebView)
			{
				this.stageWebView.stage = null;
			}
			this.removeEventListener(Event.ENTER_FRAME, this.webView_enterFrameHandler);
		}

		/**
		 * @private
		 */
		protected webView_enterFrameHandler(event:Event):void
		{
		 ...
		 * @private
		 */
		protected stageWebView_errorHandler(event:ErrorEvent):void
		{
		 ...
		 * @private
		 */
		protected stageWebViewHandler(event:Event.events.Event):void
		{
