...
module feathers.controls.text
{
	...
	import StageTextField = feathers.text.StageTextField;
	 ...
	 */
	export class StageTextTextEditor extends FeathersControl implements IMultilineTextEditor
	{
		/**
		 * @private
		 */
		 ...
		 * @private
		 */
		protected static INVALIDATION_FLAG_POSITION:string = "position";
		...
		constructor()
		{
			...
			this.addEventListener(this.starling.events.Event.REMOVED_FROM_STAGE, this.textEditor_removedFromStageHandler);
		}
		...
		/*override*/ public set x(value:number)
		{
			...
			this.invalidate(StageTextTextEditor.INVALIDATION_FLAG_POSITION);
		}
		 ...
		 * @private
		 */
		protected _ignoreStageTextChanges:boolean = false;
		 ...
		 * @private
		 */
		protected _measureTextField:TextField;
		 ...
		 */
		protected _stageTextIsTextField:boolean = false;

		/**
		 * @private
		 */
		protected _stageTextHasFocus:boolean = false;
		 ...
		 * @private
		 */
		protected _stageTextIsComplete:boolean = false;
		 ...
		 * @private
		 */
		protected _autoCapitalize:string = "none";
		 ...
		 * @private
		 */
		protected _autoCorrect:boolean = false;
		 ...
		 * @private
		 */
		protected _disabledColor:number = 0x999999;
		 ...
		 * @private
		 */
		protected _fontFamily:string = null;
		 ...
		 * @private
		 */
		protected _fontPosture:string = FontPosture.NORMAL;
		 ...
		 * @private
		 */
		protected _fontWeight:string = FontWeight.NORMAL;
		 ...
		 * @private
		 */
		protected _returnKeyLabel:string = "default";
		 ...
		 * @private
		 */
		protected _softKeyboardType:string = "default";
		 ...
		 * @private
		 */
		protected _textAlign:string = TextFormatAlign.START;
		...
		/*override*/ public render(support:RenderSupport, parentAlpha:number):void
		{
			...
			StageTextTextEditor.HELPER_POINT.x = StageTextTextEditor.HELPER_POINT.y = 0;
			this.getTransformationMatrix(this.stage, StageTextTextEditor.HELPER_MATRIX);
			MatrixUtil.transformCoords(StageTextTextEditor.HELPER_MATRIX, -desktopGutterPositionOffset, -desktopGutterPositionOffset, StageTextTextEditor.HELPER_POINT);
			...
			stageTextViewPort.x = Math.round(starlingViewPort.x + (StageTextTextEditor.HELPER_POINT.x * scaleFactor));
			stageTextViewPort.y = Math.round(starlingViewPort.y + (StageTextTextEditor.HELPER_POINT.y * scaleFactor));
			this.stageText.viewPort = stageTextViewPort;

			if(this.stageText.visible)
			{
				this.getTransformationMatrix(this.stage, StageTextTextEditor.HELPER_MATRIX);
				var globalScaleX:number = matrixToScaleX(StageTextTextEditor.HELPER_MATRIX);
				var globalScaleY:number = matrixToScaleY(StageTextTextEditor.HELPER_MATRIX);
			...
			if(this.textSnapshot)
			{
				this.textSnapshot.x = Math.round(StageTextTextEditor.HELPER_MATRIX.tx) - StageTextTextEditor.HELPER_MATRIX.tx - desktopGutterPositionOffset;
				this.textSnapshot.y = Math.round(StageTextTextEditor.HELPER_MATRIX.ty) - StageTextTextEditor.HELPER_MATRIX.ty - desktopGutterPositionOffset;
			}
				...
				if(position)
				{
					...
					else
					{
						...
						if(this._pendingSelectionBeginIndex < 0)
						{
							if(this._multiline)
							{
								var lineIndex:number = int(positionY / this._measureTextField.getLineMetrics(0).height);
								try
								{
		...
		protected layout(sizeInvalid:boolean):void
		{
			...
			var stylesInvalid:boolean = this.isInvalid(this.INVALIDATION_FLAG_STYLES);
			var dataInvalid:boolean = this.isInvalid(this.INVALIDATION_FLAG_DATA);
			var positionInvalid:boolean = this.isInvalid(StageTextTextEditor.INVALIDATION_FLAG_POSITION);
		...
		protected autoSizeIfNeeded():boolean
		{
			...
			this.measure(StageTextTextEditor.HELPER_POINT);
			return this.setSizeInternal(StageTextTextEditor.HELPER_POINT.x, StageTextTextEditor.HELPER_POINT.y, false);
		}

		/**
		 * @private
		 */
		protected refreshMeasureProperties():void
		{
			var nativeScaleFactor:number = 1;
			if(Starling.current.supportHighResolutions)
			{
		 ...
		 * @private
		 */
		protected refreshStageTextProperties():void
		{
		...
		protected doPendingActions():void
		{
			...
			if(this._pendingSelectionBeginIndex >= 0)
			{
				...
				var endIndex:number = (this._pendingSelectionEndIndex < 0) ? this._pendingSelectionBeginIndex : this._pendingSelectionEndIndex;
				this._pendingSelectionBeginIndex = -1;
				this._pendingSelectionEndIndex = -1;
		...
		protected refreshSnapshot():void
		{
			...
			this.getTransformationMatrix(this.stage, StageTextTextEditor.HELPER_MATRIX);
			this.textSnapshot.scaleX = 1 / matrixToScaleX(StageTextTextEditor.HELPER_MATRIX);
			this.textSnapshot.scaleY = 1 / matrixToScaleY(StageTextTextEditor.HELPER_MATRIX);
		 ...
		 * @private
		 */
		protected refreshViewPort():void
		{
			var starlingViewPort:Rectangle = Starling.current.viewPort;
			var stageTextViewPort:Rectangle = this.stageText.viewPort;
			if(!stageTextViewPort)
		 ...
		 * @private
		 */
		protected disposeStageText():void
		{
			if(!this.stageText)
			{
				return;
			}
			this.stageText.removeEventListener(this.flash.events.Event.CHANGE, this.stageText_changeHandler);
			...
			this.stageText.removeEventListener(this.flash.events.Event.COMPLETE, this.stageText_completeHandler);
		 ...
		 * with a custom implementation.</p>
		 */
		protected createStageText():void
		{
			...
			this.stageText.visible = false;
			this.stageText.addEventListener(this.flash.events.Event.CHANGE, this.stageText_changeHandler);
			...
			this.stageText.addEventListener(this.flash.events.Event.COMPLETE, this.stageText_completeHandler);
			this.invalidate();
		}
		 ...
		 * @private
		 */
		protected textEditor_removedFromStageHandler(event:Eventing.events.Event):void
		{
		 ...
		 * @private
		 */
		protected stageText_changeHandler(event:Event.events.Event):void
		{
		 ...
		 * @private
		 */
		protected stageText_completeHandler(event:Event.events.Event):void
		{
			this.stageText.removeEventListener(this.flash.events.Event.COMPLETE, this.stageText_completeHandler);
			this.invalidate();
		 ...
		 * @private
		 */
		protected stageText_focusInHandler(event:FocusEvent):void
		{
			this._stageTextHasFocus = true;
			this.addEventListener(this.starling.events.Event.ENTER_FRAME, this.hasFocus_enterFrameHandler);
			if(this.textSnapshot)
			{
		 ...
		 * @private
		 */
		protected stageText_focusOutHandler(event:FocusEvent):void
		{
		 ...
		 * @private
		 */
		protected hasFocus_enterFrameHandler(event:Eventing.events.Event):void
		{
			if(this._stageTextHasFocus)
			{
			...
			{
				this.removeEventListener(this.starling.events.Event.ENTER_FRAME, this.hasFocus_enterFrameHandler);
			}
		}
		 ...
		 * @private
		 */
		protected stageText_keyDownHandler(event:KeyboardEvent):void
		{
		 ...
		 * @private
		 */
		protected stageText_keyUpHandler(event:KeyboardEvent):void
		{
			if(!this._multiline && (event.keyCode == Keyboard.ENTER || event.keyCode == Keyboard.NEXT))
			{
				event.preventDefault();
		 ...
		 * @private
		 */
		protected stageText_softKeyboardActivateHandler(event:SoftKeyboardEvent):void
		{
		 ...
		 * @private
		 */
		protected stageText_softKeyboardDeactivateHandler(event:SoftKeyboardEvent):void
		{
